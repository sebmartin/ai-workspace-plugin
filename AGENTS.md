...
|---|---|---|---|
| MCP server code | `skills/threads/scripts/mcp_server.py` | `.claude-plugin/plugin.json` mcpServers | `.codex-plugin/plugin.json` mcpServers |
...
- Codex native: `${PLUGIN_ROOT}`, `${PLUGIN_DATA}` (Codex also aliases the Claude names for compat)
...
- Accessed via `get_template_path()` in `skills/common/workspace_utils.py` or the `get_template` MCP tool
