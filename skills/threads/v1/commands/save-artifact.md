...
If the artifact is meant to be sent or shown to someone without access to this workspace, it must stand alone: no decision IDs, session paths, thread names, or "see the thread". Carry over the reasoning the reader needs instead of the pointer, and only what they're entitled to read. See "Anything that leaves the workspace must stand alone" in the skill's Behavioral Principles.
