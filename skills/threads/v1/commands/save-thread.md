...
If you find unlogged decisions, present them all at once: "I noticed decisions about [X], [Y], [Z] — want me to log any of these?" Wait for one response. Log the confirmed ones using the `log-decision` command, which creates the files and links them immediately.
