...
That boundary is load-bearing in two places. `link-thread` maintains
`**Parent Thread**` / `**Child Threads**` / `**Related Threads**` by editing the
README of both threads it connects, and it is the one operation that inherently
spans shapes since either side may be schema 1 or 2. And `archive_thread` greps
`**Started**:` and `**Last Session**:` out of the README; if a render dropped
them, archiving would fall through to a `st_ctime` fallback that reports the
migration date for any migrated thread.
"""
